...
From https://github.com/junxiaosong/AlphaZero_Gomoku/blob/master/train.py
...
class NeuralNet(nn.Module):
    ...
    def __init__(self, board_size):
        ...
        # common layers
        self.cnn1d_1 = nn.Conv1d(15,10,1)
        self.lin = nn.Linear(board_size, 32)
        self.lin2 = nn.Linear(32, 14)
        self.lin3 = nn.Linear(32, 1)

        self.conv1 = nn.Conv1d(15, 32, kernel_size=3)  #, padding=1)
        ...
        self.val_fc1 = nn.Linear(2*board_size, 64)
        self.val_fc2 = nn.Linear(64, 1)

    def forward(self, state_input):
        x = F.relu(self.lin(state_input))
        x_act = F.log_softmax(self.lin2(x))
        x_val = F.tanh(self.lin3(x))
...
class PolicyValueNet:
    ...
    def __init__(self, board_size, model_file=None, use_gpu=False):
        ...
        if self.use_gpu:
            self.policy_value_net = NeuralNet(board_size).cuda()
        else:
            self.policy_value_net = NeuralNet(board_size)
